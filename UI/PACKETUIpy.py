...
class Application(tk.Frame):
    ...
    def __init__(self, master=None):
        ...
        self.Voicetext = ["PACKET DATA ", "Personal or Group", "Source Address", "Target Address", "Encrypt or not", "Others",
                          "NO way"]
        self.Labelm = [1, 2, 3, 4, 5, 6, 7];
        self.Encrym = [1, 2, 3, 4, 5, 6, 7];
        self.root = master
        self.PACKETCustomeLive = True
        self.ifRun = True
        self.pack()
        #self.createWidgets()
        self.packLabel()
    ...
    def PACKETCustomer(self,clerk):
        while self.PACKETCustomeLive:
            if self.ifRun:
                #print("UDT Customer Run")
                haha = clerk.get()
                print("PACKET UI get 到了数据：（{}）".format(haha))
